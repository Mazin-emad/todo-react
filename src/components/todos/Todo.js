...
const Todo = ( {todo, toggleTodo, del, editTodos, mode} ) => {
  return (
    ...
    <div className={`todos-todo ${todo.done ? 'done' : ''}`}>
      ...
      {mode !== "edit" &&(
        <div className='todos-todo_cta'>
          <div className='todos-todo_cta-edit' onClick={()=> editTodos(todo)}>
            <FeatherIcon icon='edit' size={20} />
          </div>
          <div className='todos-todo_cta-delete' onClick={()=>del(todo.id)}>
